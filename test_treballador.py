import __init__.Treballador as Treballador
import unittest
...
class TreballadorTest(unittest.TestCase):
    ...
    def test_tipus_treballador_incorrecte(self):
        tipus_treballador_incorrecte = "INVALID"
        treballador = Treballador()

        # Utilitzem el mètode set_tipus_treballador que pot provocar una excepció
        excepcio_meva = self.assertRaises(Exception, lambda: treballador.set_tipus_treballador(tipus_treballador_incorrecte))

        self.assertEqual("Tipus de treballador no vàlid", excepcio_meva.message)
        # L'asserció mira si el text de l'excepció avisa del problema de tipus de treballador incorrecte
